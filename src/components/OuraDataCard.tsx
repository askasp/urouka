  ...
  const Column = ({ uro }: { uro: Uromaker2 }) => (
    <>
      <div className="textContainer">
        <img
          alt=""
          style={{ borderRadius: "50%", width: "12vim", height: "12vmin" }}
          src={uro.img}
        />
      </div>
      {Days.map((day, index) => (
    ...
    <div className="root">
      ...
      <div className="uroCardTop">
        <div id="container">
          ...
          {uromakers.map((uromaker, index) => (
            <div>
              {" "}
              <Column uro={uromaker} key={index} />{" "}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="uroCardBottom">
        <div id="container">
          ...
          {individualavg.map((avg, index) => (
            <div>
              <>
                <div className="textContainer">
                  {index + 1 === individualavg.length ? (
                    <h2 style={{ borderRight: "0px" }}> {avg} </h2>
                  ) : (
                    <h2>{avg} </h2>
                  )}
                </div>

...
export async function GetOuraData(token: string, dataType: string) {
    ...
    const response = await fetch('https://api.ouraring.com/v1/' + dataType + "?start=" + prevSunday + "&end=" + today,
        {
            headers: {
                'Authorization': `Bearer ${token}`,
            },
...
export async function UpdateUserData(uromakers2: Uromaker2[], token: string, userid: string) {
